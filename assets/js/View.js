class View {
    ...
    versionsEditor(numberOfVersions, originalText, preparedText) {
        ...
        for (let i = 0; i < numberOfVersions; i++) {
            document.getElementById('accordion').innerHTML += '<div id="version' + (i + 2) + '" class="versionBlock my-3 card bg-blue text-snow"><div class="d-flex flex-row justify-content-center bg-blue align-items-baseline"><input class="d-none mr-2" type = "checkbox" name = "solidVersion' + (i + 2) + 'Checker" id = "solidVersion' + (i + 2) + 'Checker" ><div class="p-0 d-flex align-items-baseline justify-content-start letterNameBlock" data-version="' + (i + 2) + '"><input type="text" data-version="' + (i + 2) + '" class="d-none letterNameEditor"><h3 class="modal-title text-snow letterName d-inline" id="heading' + (i + 1) + '" data-version="' + (i + 2) + '"  data-toggle="collapse" data-target="#collapse' + (i + 1) + '">Version' + (i + 2) + '</h3><button class="badge  bg-snow mx-2 my-4 letterNameBadge" data-version="' + (i + 2) + '"><span><img src="./assets/icons/editor.svg" alt="edit"></span></button></div><button class="text-blue btn  bg-snow text-blue ml-5" data-toggle="collapse" data-target="#collapse' + (i + 1) + '">&darr;</button></div></div><div id="collapse' + (i + 1) + '" class="collapse" data-parent="#accordion" aria-labelledby="heading' + i + '"><div class="card-body body p-1 p-md-3  border border-info rounded-bottom" data-content="' + (i + 1) + '">' + preparedText + '</div></div>';
